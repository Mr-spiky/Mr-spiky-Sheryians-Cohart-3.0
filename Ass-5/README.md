# OLIV (Responsive Web Application)

[![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat-square&logo=html5&logoColor=white)](https://developer.mozilla.org/en-US/docs/Web/HTML)
[![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)](https://developer.mozilla.org/en-US/docs/Web/CSS)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg?style=flat-square)](https://opensource.org/licenses/MIT)

A premium, fully responsive web application built purely with semantic HTML5 and modern CSS3. Designed with a mobile-first approach to ensure flawless user experiences across all device form factors.

---

## 🚀 Live Demo

Check out the live deployment of the project here: [Live Demo](https://www.mrspiky.me/Mr-spiky-Sheryians-Cohart-3.0/Ass-5/)

---

## ✨ Features

* **100% Responsive Design:** Tailored layouts for Mobile, Tablet, and Desktop screens using CSS Flexbox and Grid.
* **Semantic Structure:** Clean, SEO-friendly HTML5 markup for better accessibility.
* **Pixel-Perfect UI:** Modern aesthetics featuring smooth transitions, hover effects, and clean typography.
* **Performance Optimized:** Lightweight architecture with no heavy external framework dependencies.

---

## 🛠️ Tech Stack

* **Structure:** HTML5 (Semantic tags like `<section>`, `<article>`, `<nav>`)
* **Styling:** CSS3 (Custom properties/variables, Flexbox, CSS Grid, Media Queries)
* **Icons & Fonts:** Google Fonts

---

## 📂 Project Structure

```text
Ass-5/
├── assets/          # Images, logos, and media files
├── index.html       # Main landing/entry HTML page
├── style.css        # Core stylesheet containing all responsive rules
└── README.md        # Documentation
```

### Prerequisites
You only need a modern web browser (e.g., Chrome, Edge, Safari, Firefox).

### Installation
Clone the repository:
```bash
git clone https://github.com/Mr-spiky/Mr-spiky-Sheryians-Cohart-3.0.git
cd Ass-5
```

---

## 📝 License
Distributed under the MIT License. See LICENSE for more information.

---

## 👤 Author

**Shivam Kumar**

* GitHub: [@Mr-spiky](https://github.com/Mr-spiky)
